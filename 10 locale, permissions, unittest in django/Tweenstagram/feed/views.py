...
def home(request):
    ...
    if request.method == "POST":
        if not request.user.has_perm("feed.add_tweet"):
...
@permission_required("feed.add_tweet", raise_exception=True)
def settings(request):
    if request.method == "POST":
        f = SettingsForm(request.POST)
        if f.is_valid():
            return HttpResponse(_("GOOD") + str(f.cleaned_data))
        else:
